...
<?php
	...
	if(empty($messagebody)){
		...
		$rec_message = "1/3/on/pass";   	//????????  

		$rec_msg_array = explode("/", $rec_message);
		if ($rec_msg_array[2] = "off"){
			if($rec_msg_array[3] = "pass"){
				$showlist = ResourceAllocation::found_mobile_Result_by_instanceId($rec_msg_array[0],$rec_msg_array[1],"on");
				// showlist will be show in html page (device, hostIp,deviceIp,brand/api, stauts,)
			}
			else{
				$_SESSION["message"] = "Your request to start the instance is failed, please try again";			
			}
