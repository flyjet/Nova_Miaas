<?php include("../includes/layouts/header.php"); ?>

			<!-- row 1 start in header -->

		        <div class="col-lg-8">
				   	<ul class="nav navbar-nav navbar-right">
				        <li><a href="register.php" >Sign Up </a></li>
				       	<li><a href="login.php" >Sign In </a></li>
				    </ul>  
			    </div> 
			</header> <!-- end of class row 1-->

			<!-- row 2 -->
			<div class="row">
				<aside class="col-lg-2" style="border-right: 1px solid #E4E4E4;">  <!-- side navbar -->

		        	<h4> About Nova MIaaS</h4>
			            <ul class="side-navbar nav nav-pills nav-stacked">
			                <li><a href="readmore.php">Overview</a></li>
			                <li><a href="readmore_b">Benefits </a></li>
			                <li class="active"><a href="readmore_i.php">Instances </a></li>
			                <li><a href="readmore_p">Pricing </a></li>
			            </ul> 
	         	</aside>

	         	<article class="col-lg-offset-1 col-lg-8 ">
	         		<h1orange>Nova MIaaS Instances</h1orange>
	         		<p style="font-size:16px;"> Nova MIaaS Instances </p>


	         	</article>
	    	</div><!-- end of class row 2-->


			<!-- row 3 -->
		    <div class="col-lg-offset-4" >
		    	<br>
		    	<br> 
				<span>Get Started for Free </span>
				<a class="btn btn-warning" href="register.php">Create Free Account</a>
			</div>

			<!-- row 4 in footer -->

<?php include("../includes/layouts/footer.php"); ?>